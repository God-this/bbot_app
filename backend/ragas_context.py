# Ragas 기반 RAG 평가 모듈
#
# python ragas_context.py              # 기본 5개
# python ragas_context.py --limit 20   # 20개
# python ragas_context.py --limit -1   # 전체
#
# 전제:
#   - .env의 PROVIDER / LLM_MODEL을 원하는 "새로운 모델"로 맞춰놓고 실행
#     (judge LLM은 평가 안정성을 위해 gpt-4o-mini로 고정)
...
# ==================== Ragas용 LLM/임베딩 ====================
def _get_ragas_llm():
    """judge LLM: 답변 생성 모델과 무관하게 안정성을 위해 gpt-4o-mini 고정 권장"""
...
def save_eval_result(result: dict, path: str = EVAL_LOG_PATH):
    ...
    jsonl_path = os.path.join(path, f"eval_{date_str}.jsonl")
    with open(jsonl_path, "a", encoding="utf-8") as f:
        f.write(json.dumps(result, ensure_ascii=False) + "\n")

    txt_path = os.path.join(path, f"eval_{date_str}.txt")
    lines = [
        f"Q: {result['question']}",
        f"A: {result['answer']}",
    ]
...
def run_rag(question: str) -> tuple[str, dict]:
    """새로운 모델 평가 = .env의 PROVIDER/LLM_MODEL 값을 새 모델로 바꾼 뒤 이 함수를 그대로 호출"""
